...
const FormReport: React.FC<FormReportProps> = ({ onSubmit, images }) => {
    const {
        register,
        handleSubmit,
        formState: { errors },
    ...
    return (
        <form className="space-y-4 m-4" onSubmit={handleSubmit(onSubmit)}>
            <Input
                {...register("title")}
                label="Título"
                variant="flat"
            ...
            <button
                type="submit"
                className="w-3/6 lg:w-2/6 p-2 m-4 bg-primary text-white rounded-3xl hover:bg-accent hover:duration-300"
            >
                Enviar
            </button>
        </form>
