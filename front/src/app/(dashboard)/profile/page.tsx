...
const FormUserSchema = z.object({
    name: z
        ...
        .refine((val) => /^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/.test(val), { message: "El nombre no puede contener números ni caracteres que no sean letras." }),
    email: z.string().min(1, "Correo electrónico requerido").email("Formato de email inválido"),
    phone: z
        .string()
        .optional()
        .refine((val) => !val || /^[+]?[\d]+$/.test(val), { message: "Teléfono inválido" })
        .transform((val) => (val ? Number(val.replace("+", "")) : undefined)),
    address: z.object({
        province: z.string()
            .min(1, "Provincia requerida")
            .refine((val) => /^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/.test(val), { message: "La Provincia no puede contener números ni caracteres que no sean letras." }),
        locality: z.string()
